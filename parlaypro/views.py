...
def parlays(request): 
    ...
    return render(request, 'parlays.html', context={'balance': user_balance})
...
def submit_parlay(request):
    if request.method == 'POST':
        ...
        if 'parlay' in request.session:
            ...
            print(lines)
            cursor = connection.cursor()
            #cursor.execute('INSERT INTO aaa_parlay, 'balance': user_balance (user_id, open, number_of_legs, amount_wagered) VALUES ("{}", "{}", "{}")'.format(request.user.username, request.user.email, 0))
